...
public class DanhMucChi {
    ...
    public static void updateDanhMucChiPanel(JPanel panel_danhmuc, ButtonGroup buttonGroup1, JButton button_chinhsua1) {
    ...
    public static void populateDanhmucComboBox(JComboBox<String> Danhmuc) {
        String sql = "SELECT Name_Type FROM Type WHERE Receipts_Or_expenses = 1";
        try {
    ...
    private static void saveCategoryToDatabase(String newCategory, JComboBox<String> Danhmuc) {
        String sql = "INSERT INTO Type (Name_Type,Receipts_Or_expenses) VALUES (?, 1)";
        try {
            Connection connection = JDBCConnection.getJDBCConnection();
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
    ...
    private static boolean isCategoryAlreadyExists(String newCategory, JComboBox<String> Danhmuc) {
        ...
        for (String category : categories) {
            ...
            String sql = "SELECT COUNT(*) FROM Type WHERE UPPER(Name_Type) = UPPER(?) and (Receipts_Or_expenses = 1)";
        try {
            Connection connection = JDBCConnection.getJDBCConnection();
